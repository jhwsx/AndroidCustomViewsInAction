...
/**
 * after 函数传入的动画，在 play 函数传入的动画结束之前播放。
 *
 * @author wangzhichao
 * @since 2022/6/16
 */
class E_AnimatorSetBuilderAfterViewGroup @JvmOverloads constructor(
    ...
    init {
        inflate(context, R.layout.layout_animatorset_builder_after, this)
        ...
        button.setOnClickListener {
            ...
            animatorSet.play(tv1BgAnimator)
                // 这两个 after 还是同时播放的。
                .after(tv1TranslationX).after(tv2TranslationX)
            animatorSet.start()
        }
    }
}