...
public class PaintSetXfermodePorterDuffXfermodeViewGroup extends LinearLayout {
...

    {
        ...
        // 源图像非交合区域会保持原样。
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.ADD, "Mode.ADD", "饱和度相加，公式是Saturate(S + D)。" +
        ...
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.LIGHTEN, "Mode.LIGHTEN", "变亮模式，算法是： [Sa + Da - Sa*Da,Sc*(1 - Da) + Dc*(1 - Sa) + max(Sc, Dc)]。" +
                "只有交合区域才有 PhotoShop 变亮效果，非交合区域显示原图像。" +
                "Retains the largest component of the source and destination pixel."));
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.DARKEN, "Mode.DARKEN", "变暗模式，对应公式是： [Sa + Da - Sa*Da,Sc*(1 - Da) + Dc*(1 - Sa) + mix(Sc, Dc)]。" +
                "只有交合区域才有 PhotoShop 变暗效果，非交合区域显示原图像。" +
                "Retains the smallest component of the source and destination pixels."));
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.MULTIPLY, "Mode.MULTIPLY", "正片叠底，公式是：[Sa * Da, Sc * Dc]。" +
                "在交合区域显示 PhotoShop 的正片叠底效果，非交合区域变为透明。" +
                "Multiplies the source and destination pixels."));
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.OVERLAY, "Mode.OVERLAY", "叠加，没有对应公式。" +
                "只有交合区域才有 PhotoShop 叠加效果，非交合区域显示原图像。" +
                "Multiplies or screens the source and destination depending on the destination color."));
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.SCREEN, "Mode.SCREEN", "滤色，对应公式是：[Sa + Da - Sa * Da, Sc + Dc - Sc * Dc]。" +
        ...
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.DST_OUT, "Mode.DST_OUT", "计算公式为：[Da * (1 - Sa), Dc * (1 - Sa)]。" +
        ...
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.DST_OVER, "Mode.DST_OVER", "计算公式为：[Sa + (1 - Sa)*Da, Rc = Dc + (1 - Da)*Sc]。" +
                "与 SRC_OVER 相反，在源图像的顶部绘制目标图像。" +
                "The source pixels are drawn behind the destination pixels."));
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.DST_ATOP, "Mode.DST_ATOP", "计算公式为：[Sa, Sa * Dc + Sc * (1 - Da)]。" +
        ...
        // 4, 其他模式
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.CLEAR, "Mode.CLEAR", "计算公式：[0, 0]。" +
                "Destination pixels covered by the source are cleared to 0."));
        MODE_MODELS.add(new ModeModel(PorterDuff.Mode.XOR, "Mode.XOR", "计算公式为：[Sa + Da - Sa*Da,Sc*(1 - Da) + Dc*(1 - Sa) + min(Sc, Dc)]。" +
