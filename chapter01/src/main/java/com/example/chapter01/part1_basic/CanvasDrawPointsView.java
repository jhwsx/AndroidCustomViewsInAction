...
public class CanvasDrawPointsView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float[] pts = new float[24]; // 2 个值确定一个点，总共是 12 个点
        for (int i = 0; i < 12; i++) {
            pts[i * 2] = (float) (200 + Math.cos(Math.toRadians(i * 30)) * 100);
            pts[i * 2 + 1] = (float) (200 + Math.sin(Math.toRadians(i * 30)) * 100);
        }
        canvas.drawPoints(pts, paint);

        float[] pts2 = new float[24];
        for (int i = 0; i < 12; i++) {
            pts2[i * 2] = (float) (200 + Math.cos(Math.toRadians(i * 30)) * 100);
            pts2[i * 2 + 1] = (float) (600 + Math.sin(Math.toRadians(i * 30)) * 100);
        }
